...
class _TipsScreenState extends State<TipsScreen> {
    ...
    return CapstoneScaffold(
      ...
      child:
        ...
        stream: Firestore.instance
            ...
            .document(this.widget.destination.documentID)
            .collection('tips')
  ...
  getCrowdSourceList() async {
    doc = await Firestore.instance
        .collection('destinations')
        .document(this.widget.destination.documentID)
        .collection('tips_sourced')
        .document('tips_sourced')
        .get();
    ...
    setState(() {
      if (doc.exists) {
        ...
        var sortedKeys = mapKeys
          ..sort((k1, k2) => doc.data[k1].compareTo(doc.data[k2]));

        if (sortedKeys.length < 5)
          crowd_source =
              List.from(sortedKeys.reversed).sublist(0, sortedKeys.length);
        else
          crowd_source = List.from(sortedKeys.reversed).sublist(0, 5);

        sortedMap = LinkedHashMap.fromIterable(crowd_source,
            key: (k) => k, value: (k) => docData[k]);
      } else {
        crowd_source = [];
        sortedMap = Map();
      }
      ...
      if (DocumentSnapshot.data != null) {
        ...
        if (num_entry == null) {
          Firestore.instance
              ...
              .collection('tips_sourced')
              .document('tips_sourced')
              .updateData({global_checklist_entry: 1});
        } else {
          Firestore.instance
              .collection('destinations')
              .document(this.widget.destination.documentID)
              .collection('tips_sourced')
              .document('tips_sourced')
              .updateData({global_checklist_entry: num_entry + 1});
        }
      } else {
        Firestore.instance
            .collection('destinations')
            ...
            .collection('tips_sourced')
            .document('tips_sourced')
            .setData({global_checklist_entry: 1});
      }
    });
  }
        ...
        if (num_entry != null) {
          if (num_entry - 1 == 0) {
            Firestore.instance
                ...
                .collection('tips_sourced')
                .document('tips_sourced')
                .updateData({global_checklist_entry: FieldValue.delete()});
          } else {
            Firestore.instance
                .collection('destinations')
                .document(this.widget.destination.documentID)
  ...
  Widget buildCategoryTiles(Entry root, String docID) {
    ...
    return GestureDetector(
      ...
      child: ExpansionTile(
        ...
        title: Text(root.title),
        children: [
          buildTipsTiles(root, docID),
        ],
        trailing: GestureDetector(
            onTap: () => addTipDialog(root, docID), child: Icon(Icons.add)),
      ),
    );
  }
  ...
  Future<String> crowdSourceDialog(
      BuildContext context, Entry root, String docID) {
    TextEditingController myController = TextEditingController();

    return showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(title: Text("Crowd Sourced List:"), actions: [
            clist(root, docID),
            new FlatButton(
              child: new Text("Cancel"),
              onPressed: () {
